...
- 当前应用已完成下一阶段 v1 功能收口：详情页可编辑学习内容进度和截止日期；主页列表只展示名称、状态、进度、截止日期；阅读状态继续只用于恢复当前资料、当前笔记和分栏比例；删除资料或笔记后会清理 reading_states 中失效引用。
...
- 本阶段已验证：前端 `vitest run` 39 个测试通过、`tsc --noEmit` 通过、`vite build` 通过；Rust `cargo fmt --check` 通过、`cargo test` 17 个测试通过、`cargo clippy -- -D warnings` 通过。
- 下一步目标：用真实 App 数据手工验收“编辑进度/截止日期后重启恢复”“删除当前阅读资料后再进阅读页不报错”“删除当前笔记后阅读页选择状态清空”。
- 后续增强项：资料重命名、学习内容名称/状态/预计工时编辑、PDF 搜索/目录、阅读页滚动位置或页码保存、资料删除失败时的逐项恢复提示。
