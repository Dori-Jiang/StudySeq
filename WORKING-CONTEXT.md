...
- 详情页是当前主要学习工作台：左栏资料支持点击整行内嵌阅读 txt、图片、PDF，右栏继续管理和编辑笔记；旧阅读页路由保留用于兼容。
...
- 当前应用已完成阅读页最小闭环：详情页左侧可内嵌阅读 txt/图片/PDF，旧阅读页仍可进入；右侧查看/创建/编辑纯文本笔记，并保存当前资料、当前笔记、分栏比例。
- 当前应用已完成阅读体验收口与数据安全补强：切换笔记和返回详情前自动保存当前笔记；PDF 支持 A4 页框、上一页、下一页、页码显示、按钮缩放、Ctrl+鼠标滚轮缩放、中键拖动；返回详情前保存阅读状态。
- 当前应用已完成 v1 工程收口：主页学习内容栏显示进度条；主页可行内编辑学习内容名称、状态、预计工时、截止日期、进度；详情页保留学习内容展示、资料和笔记；阅读状态继续只用于恢复当前资料、当前笔记和分栏比例；删除资料或笔记后会清理 reading_states 中失效引用。
